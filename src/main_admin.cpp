...
void menuAdmin(ListSales &LS, ListMobil &LM, ListRelasi &LR, long long &totalPendapatan)
{
    ...
    do
    {
        ...
        case 1:
            ...
            cout << "Masukkan Nama: ";
            cin >> S.nama;
            cout << "Jumlah mobil yg SUDAH terjual: ";
            cin >> S.jumlahTerjual;
            PS = alokasiSales(S);
            ...
            if (subPilihan == 1)
                ...
                insertLastSales(LS, PS);
            else
            {
                cout << "ID Sales sebelumnya: ";
                cin >> idPrec;
                PrecSales = findSales(LS, idPrec);
                if (PrecSales)
                    insertAfterSales(LS, PrecSales, PS);
                else
                {
        ...
        case 4:
            ...
            if (PS)
            {
                ...
                if (PS == LS.first)
                    deleteFirstSales(LS, PS);
                else if (PS == LS.last)
                    deleteLastSales(LS, PS);
                else
                {
                    adrSales temp = PS->prev;
                    deleteAfterSales(LS, temp, PS);
                }
                delete PS;
        ...
        case 5:
            ...
            if (PM)
            {
                ...
                if (PM == LM.first)
                    deleteFirstMobil(LM, PM);
                else if (PM->next == nullptr)
                    deleteLastMobil(LM, PM);
                else
                {
        ...
        case 7:
            ...
            else
            {
                ...
                PM = findMobil(LM, idM);
                if (PM)
                    cout << "Ketemu: " << PM->info.model << endl;
                else
                    cout << "404 Not Found" << endl;
            }
            break;
        ...
        case 8:
            ...
            cin >> idM;
            cout << "Masukkan ID Sales yang menjual: ";
            cin >> idS;

            PM = findMobil(LM, idM);
            PS = findSales(LS, idS);

            if (PS != nullptr && PM != nullptr)
            {
                adrRelasi cekRelasi = findRelasi(LR, PS, PM);
                if (cekRelasi != nullptr)
                {
